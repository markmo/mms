...
public class Table extends NamedColumnSet {
    ...
    public static Table parseTable(ObjectMapper mapper, JsonNode json, Namespace namespace) throws IOException {
        ...
        table.setName(name);
        TableType tableType = mapper.readValue(json.path("tableType").binaryValue(), TableType.class);
        ...
        TableRole tableRole = mapper.readValue(json.path("role").binaryValue(), TableRole.class);
        ...
        table.setRowCount(json.path("rowCount").longValue());
        ...
        if (columnsJson.size() > 0) {
            ...
            Iterator<JsonNode> it = columnsJson.elements();
            while (it.hasNext()) {

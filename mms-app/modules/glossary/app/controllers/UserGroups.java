...
import models.domain.business.UserGroup;
...
public class UserGroups extends Controller {
    ...
    public static Result index(int pageIndex, int pageSize, String sortBy, String order) {
        ...
        links.add(String.format("<%s&total_entries=%d>; rel=\"first\"",
                controllers.glossary.routes.UserGroups.index(1, pageSize, sortBy, order), totalRowCount));
        links.add(String.format("<%s&total_entries=%d>; rel=\"last\"",
                controllers.glossary.routes.UserGroups.index(page.getLastPageIndex(), pageSize, sortBy, order), totalRowCount));
        if (page.hasPrev()) {
            links.add(String.format("<%s&total_entries=%d>; rel=\"prev\"",
                    controllers.glossary.routes.UserGroups.index(pageIndex - 1, pageSize, sortBy, order), totalRowCount));
        }
